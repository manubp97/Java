...
public abstract class AlgoritmoGenetico {
	...
	public Individuo ejecuta() {
		for (int i = 0; i < pasos; i++) {
			...
			Cromosoma c1 = poblacion.individuo(a1).cromosoma();
			Cromosoma c2 = poblacion.individuo(a2).cromosoma();
			Cromosoma c3 = recombinar(c1, c2);
			c3.mutar(probMutacion);
			poblacion.reemplaza(new Individuo(c3, problema));
		}
